...
export class UserInfoRenderer {
  ...
  async renderUserInfo(user: any) {
    ...
    userInfoDiv.innerHTML = `
      <div class="flex flex-col xl:flex-row items-center justify-between space-y-8 xl:space-y-0 xl:space-x-12 w-full">
        <!-- User Profile Section -->
        <div class="flex flex-col lg:flex-row items-center space-y-6 lg:space-y-0 lg:space-x-8 w-full xl:w-auto">
          ...
          <div class="relative group flex-shrink-0">
            ...
            <img src="${getUserImageSrc()}" 
                 ...
                 class="relative w-28 h-28 sm:w-36 sm:h-36 rounded-full object-cover border-4 border-white/20 shadow-2xl 
            ...
            <div class="absolute -bottom-1 -right-1 bg-gradient-to-r from-green-400 to-emerald-500 w-8 h-8 rounded-full border-4 border-white/30 shadow-xl flex items-center justify-center backdrop-filter backdrop-blur-sm">
              <div class="w-3 h-3 bg-white rounded-full animate-pulse"></div>
            </div>
          </div>
          
          <!-- Enhanced User Details -->
          <div class="text-center lg:text-left w-full max-w-3xl space-y-4">
            <div class="space-y-2">
              <h2 class="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-orange-300 via-red-300 to-pink-300 bg-clip-text text-transparent break-words leading-tight">
                ${user.fullName || 'Player'}
              </h2>
              <p class="text-xl sm:text-2xl text-orange-300 font-semibold tracking-wide">@${user.nickName || 'Unknown'}</p>
            </div>
            
            <!-- Enhanced User Meta Information -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
              <div class="flex items-center justify-center lg:justify-start space-x-3 p-3 rounded-xl bg-white/5 backdrop-filter backdrop-blur-sm border border-white/10">
                ...
                <div class="text-left">
                  <p class="text-sm text-gray-400 font-medium">${i18n.t('dashboard.joined')}</p>
                  <p class="text-gray-200 font-semibold">${user.joinedAt ? new Date(user.joinedAt).toLocaleDateString() : 'Unknown'}</p>
                </div>
              </div>
              
              <div class="flex items-center justify-center lg:justify-start space-x-3 p-3 rounded-xl bg-white/5 backdrop-filter backdrop-blur-sm border border-white/10">
                <div class="w-10 h-10 rounded-full bg-gradient-to-r from-green-400 to-teal-400 flex items-center justify-center flex-shrink-0">
                  <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="text-left">
                  <p class="text-sm text-gray-400 font-medium">${i18n.t('dashboard.email')}</p>
                  <p class="text-gray-200 font-semibold truncate max-w-48">${user.email || 'Unknown'}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="flex-shrink-0 flex justify-center items-center w-full xl:w-auto">
          <div class="p-6 rounded-2xl bg-white/5 backdrop-filter backdrop-blur-sm border border-white/10">
