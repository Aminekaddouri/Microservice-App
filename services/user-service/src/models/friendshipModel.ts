...
async function createFriendRequest(userA: string, userB: string): Promise<Friendship> {
...
async function changeFriendshipStatus(userA: string, userB: string, status: string): Promise<void> {
...
async function deleteFriendship(userA: string, userB: string): Promise<void> {
    ...
    await db.run(
        `DELETE FROM Friendship WHERE user1ID = ? AND user2ID = ?`,
        [user1ID, user2ID]
    );
}

async function getFriendship(userA: string, userB: string): Promise<Friendship | null> {
    const db = await openDB();
    const [user1ID, user2ID] = normalizePair(userA, userB);
...
async function getUserFriends(userId: string, status: FriendshipStatus): Promise<Friendship[]> {
    const db = await openDB();
...
async function deleteFriendshipById(friendshipId: string, userId: string) {
    try {
        ...
        // Check if the user has permission (either user1ID or user2ID)
        if (friendship.user1ID !== userId && friendship.user2ID !== userId) {
            throw new Error('You do not have permission to delete this friend request');
        }
